...
class ToolTip(object):
    ...
    def showtip(self, text):
        ...
        label = ttk.Label(tw, text=self.text, justify=tk.LEFT,background="#ffffff", relief=tk.SOLID, borderwidth=1,font=("segoe", "8", "normal"))
        label.pack(ipadx=1)
    def hidetip(self):
        tw = self.tipwindow
        self.tipwindow = None
...
class BetterCheckMenu:
    def __init__(self,parent,vals,vars,*separators,**kwargs):
        self._popup = None
        self.text = None
        self._menubutton = []
        self.parent = parent
    ...
    def on_popup(self, event):
        if not self._popup and not self.text:
            ...
            self.text=tk.Text(self._popup,width=self.ceil(max([len(str(i))+4 for i in self.vals])*3/4),background='#f0f0f0')
            if len(self.vals)+len(self.seps)<15:
                self.text.configure(height=int(11/8*(len(self.vals)+len(self.seps))))
            ...
            c=0
            for i in range(len(self._menubutton)):
                if self.seps and len(self.seps)>0 and c<len(self.seps):
                    if self.seps[c][1]==i:
                        j=ttk.Label(self.text,text=self.seps[c][0])
                        c+=1
                        self.text.window_create('end',window=j)
                        self.text.insert('end','\n')
                j=ttk.Checkbutton(self.text,text=self._menubutton[i]['label'],variable=self._menubutton[i]['variable'])
                if i!=len(self._menubutton)-1:
    ...
    def destroy(self,event):
        ...
        self.selb=((self._popup.winfo_rootx(),self._popup.winfo_rooty()),(self._popup.winfo_rootx()+self._popup.winfo_width(),self._popup.winfo_rooty()+self._popup.winfo_height()))
        if not ((self.pb[0][0]<=self.parent.winfo_pointerxy()[0]<=self.pb[1][0] and self.pb[0][1]<=self.parent.winfo_pointerxy()[1]<=self.pb[1][1]) or (self.selb[0][0]<=self.parent.winfo_pointerxy()[0]<=self.selb[1][0] and self.selb[0][1]<=self.parent.winfo_pointerxy()[1]<=self.selb[1][1])):
            if self._popup:
                windo=self._popup
                self._popup=None
                windo.destroy()
...
def handback(c,sc,d,tour,tb,tint,sa):
    global cc,sscc,dd,ttoouurr,ttbb,tthhyymmee,savestats
    cc,sscc,dd,ttoouurr,ttbb,tthhyymmee,savestats=c,sc,d,tour,tb,tint,sa
    return
def gencard():
    s=''
    ...
    if len(bonlist)!=0 and len(bonlist)!=bonstatus.count('skipped'):
        ...
        for i in range(len(bonlist)):
            ...
            bstr+="\n"
        s+=bstr
    if (len(tulist)!=0 or len(bonlist)!=0) and len(s.strip())>0:
...
def setup():
    ...
    fram1=ttk.Frame(root)
    fram1.grid(row=0,column=0)
    cats=[tk.IntVar() for i in range(len(ccccc))]
    catl=ttk.Label(fram1, text = 'Categories: ', font=('calibri',10, 'bold'))
    catt=ttk.Menubutton (fram1, text="Click to select options")
    if cc!=ccccc and cc:
        for i in cc:
            cats[ccccc.index(i)].set(1)
    catt.menu=BetterCheckMenu(catt,ccccc,cats)
    catt['menu']=catt.menu
    lcats=ttk.Button(fram1, text='❔', width=2)
    allsubcatlist=[]
    for i in sssssccccc:
        allsubcatlist.extend(sssssccccc[i])
    subcats=[tk.IntVar() for i in range(len(allsubcatlist))]
    subcatl=ttk.Label(fram1, text = 'Subcategories: ', font=('calibri',10, 'bold'))
    subcatt=ttk.Menubutton (fram1, text="Click to select options")
    if sscc:
        for i in sscc:
            subcats[allsubcatlist.index(i)].set(1)
    ooooooooooooo=[]
    for i in sorted(sssssccccc.keys()):
        ooooooooooooo.append((i,allsubcatlist.index(sssssccccc[i][0])))
    subcatt.menu =  BetterCheckMenu(subcatt,allsubcatlist,subcats,ooooooooooooo)
    subcatt["menu"] = subcatt.menu
    lsubcats=ttk.Button(fram1, text='❔', width=2)
    diffs=[tk.IntVar() for i in range(len(ddddd))]
    diffl=ttk.Label(fram1, text = 'Difficulties: ', font=('calibri',10, 'bold'))
    difft=ttk.Menubutton (fram1, text="Click to select options")
    if dd!=ddddd and dd:
        for i in dd:
            diffs[i-1].set(1)
    difft.menu=BetterCheckMenu(difft,ddddd,diffs)
    difft['menu']=difft.menu
    ldiffs=ttk.Button(fram1, text='❔', width=2)
    alltourlist=[]
    for i in tttttooooouuuuurrrrr:
        alltourlist.extend(tttttooooouuuuurrrrr[i])
    tours=[tk.IntVar() for i in range(len(alltourlist))]
    tourl=ttk.Label(fram1, text = 'Tournaments: ', font=('calibri',10, 'bold'))
    tourt=ttk.Menubutton (fram1, text="Click to select options")
    if ttoouurr:
        for i in ttoouurr:
            tours[alltourlist.index(i)].set(1)
    ppppppppppppp=[]
    for i in sorted(tttttooooouuuuurrrrr.keys()):
        ppppppppppppp.append((i,alltourlist.index(tttttooooouuuuurrrrr[i][0])))
    tourt.menu =  BetterCheckMenu(tourt,alltourlist,tours,ppppppppppppp)
    tourt["menu"] = tourt.menu
    ltours=ttk.Button(fram1, text='❔', width=2)
    ...
    fram2=ttk.Frame(root)
    fram2.grid(row=1,column=0)
    timeo=ttk.Label(fram2, text = 'Time between each word (ms): ', font=('calibri',10, 'bold'))
    times=tk.IntVar()
    timenter=ttk.Entry(fram2,width=3,textvariable=times,)
    ...
    if type(tthhyymmee)==int:
        ...
        thyme.set(250)
    timeex=ttk.Button(fram2, text='❔', width=2)
    ...
    fram3=ttk.Frame(root)
    fram3.grid(row=2,column=0)
    tubon=tk.IntVar()
    if ttbb:
        tubon.set(ttbb)
    tuorbon=ttk.Label(fram3, text = 'What kind of questions do you want? ', font=('calibri',10, 'bold'))
    tu=ttk.Radiobutton(fram3, text='Tossups only', variable=tubon, value=0)
    bon=ttk.Radiobutton(fram3, text='Bonuses only', variable=tubon, value=1)
    tunbon=ttk.Radiobutton(fram3, text='Tossups and bonuses', variable=tubon, value=2)
    tubonex=ttk.Button(fram3, text='❔', width=2)
    def submit():
        ...
        tint=thyme.get()
        sa=sav.get()
        handback(c,sc,d,t,tb,tint,sa)
        root.destroy()
    fram4=ttk.Frame(root)
    fram4.grid(row=4,column=0)
    abt=ttk.Button(fram4,text = 'About')
    ...
    contr=ttk.Button(fram4,text = 'Controls')
    sumbit=ttk.Button(fram4,text = 'Go [Enter]', command = submit)
    ...
    quibt=ttk.Button(fram4,text = 'Quit [Esc/Ctrl+W]', command = leavehomescreen)
    ...
    fram5=ttk.Frame(root)
    sav=tk.BooleanVar(value=savestats)
    save=ttk.Checkbutton(fram5,variable=sav,text='Save my stats to a text file')
    save.grid(row=0,column=0)
    fram5.grid(row=3,column=0)
    saveex=ttk.Button(fram5, text='❔',width=2)
...
def qscreen(tuorbon,timeint):
    ...
    statframe=ttk.LabelFrame(topf, text='Stats')
    statframe.grid(row=0,column=0)
    pstatframe=ttk.Frame(statframe)
    pstatframe.grid(row=0,column=0)
    cstatframe=ttk.Frame(statframe)
    cstatframe.grid(row=1,column=0)
    tstatframe=ttk.Frame(statframe)
    ...
    bframe=ttk.LabelFrame(root, text='Controls')
    bframe.grid(row=2,column=0)
    controlframe=ttk.Frame(bframe)
    controlframe.grid(row=0,column=0)
    qbt=ttk.Button(topf,text='Back [Esc]',command=backtohomescreen)
    lbt=ttk.Button(topf,text='Leave [Ctrl+W]',command=leavereader)
    buzzed=False
    tuct=ttk.Label(pstatframe,text='Tossups: %s'%(tu))
    tossuppts=ttk.Label(pstatframe,text='Tossup Points: %s'%(tupts))
    ppg=ttk.Label(pstatframe,text='PPTUH: %s'%(0.0 if tu==0 else round(tupts/tu,2)))
    ptnct=ttk.Label(pstatframe,text='Powers/10s/Negs: %s'%('/'.join(str(i) for i in ptn)))
    bonct=ttk.Label(pstatframe,text='Bonuses: %s'%(bon))
    bonuspts=ttk.Label(pstatframe,text='Bonus Points: %s'%(bpts))
    ppb=ttk.Label(pstatframe,text='PPB: %s'%(0.0 if bon==0 else round(bpts/bon,2)))
    tttb=ttk.Label(pstatframe,text='30s/20s/10s/0s: %s'%('/'.join(str(i) for i in bagels)))
    eler=ttk.Label(cstatframe,text='Average Celerity: %s'%(round(sum(elerity)/len(elerity),3) if len(elerity)>0 else None))
    celer=ttk.Label(cstatframe,text='Average Correct Celerity: %s'%(round(sum(celerity)/len(celerity),3) if len(celerity)>0 else None))
    ieler=ttk.Label(cstatframe,text='Average Incorrect Celerity: %s'%(round(sum(ielerity)/len(ielerity),3) if len(ielerity)>0 else None))
    tourney=ttk.Label(tstatframe,text='Tournament: ---')
    difficul=ttk.Label(tstatframe,text='Difficulty: - (---)')
        ...
        if not ansalrgiven and not dead:
            ...
            if tbrn==0:
                ...
                if subbonnum==2:
                    ...
                    if tuorbon==2:
                        tbrn=1-tbrn
    answerline=ttk.Entry(controlframe, textvariable = is_this_correct, width=30,state='disabled')
    enterans=ttk.Button(controlframe,text="Enter [Enter]",command=checkanswer,state='disabled')
    enterans.grid(row=0,column=4)
    buzzer=ttk.Button(controlframe,text="Buzz [space]",command=buzzin)
    if reading==False:
        buzzer['state']='disabled'
        root.unbind("<space>")
    tcframe=ttk.Frame(bframe)
    tcframe.grid(row=2,column=0)
    timeo=ttk.Label(tcframe, text = 'Time between each word (ms): ', font=('calibri',10, 'bold'))
    times=tk.IntVar()
    timenter=ttk.Entry(tcframe,width=3,textvariable=times)
        ...
        if dead or ansalrgiven:
            ...
            if tuorbon==0:
                ...
                if tbrn==0:
                    ...
                    difficul['text']="Difficulty: %s (%s)"%(bontourlist[bonnum][1],diffdict[bontourlist[bonnum][1]])
                    read_bonus(qframe,qcanvas,qtext,thyme)
    read=ttk.Button(controlframe,text="Next/Skip [\]",command=readq)
    root.bind('<\>', lambda event: readq())
...
def stripemsub(s):
    for i in 'i','em','mark','small','del','ins','sub','sup','u','s','strike','big':
        s=s.replace('<%s>'%i,'')
        s=s.replace('</%s>'%i,'')
    return s
