...
class BetterCheckMenu:
    ...
    def on_popup(self, event):
        if not self._popup and not self.text:
            ...
            self.text=tk.Text(self._popup,width=max([len(str(i)) for i in self.vals])+5,background='#f0f0f0',height=min(len(self.vals)+2,10))
...
def setup():
    ...
    thyme = ttk.Scale(fram2, from_=0, to=500, orient=tk.HORIZONTAL, length=400, variable=times)
    ...
    tunbon=ttk.Radiobutton(fram3, text='Tossups and bonuses', variable=tubon, value=1)
...
def qscreen(tuorbon,timeint):
    ...
    ptnct=ttk.Label(pstatframe,text='Correct/Incorrect: %s'%('/'.join(str(i) for i in ptn)))
    ...
    tttb=ttk.Label(pstatframe,text='10s/5s/0s: %s'%('/'.join(str(i) for i in bagels)))
    ...
    thyme = ttk.Scale(tcframe, from_=0, to=500, orient=tk.HORIZONTAL, length=400, variable=times)
    thyme.set(timeint)
    ...
    def itertu(words, i, window,canvas,question_txt,timeint):
        ...
        if not buzzed:
            ...
            i += 1
            qctr = window.after(int(timeint.get()), lambda: itertu(words, i,window,canvas,question_txt,timeint))
    ...
    def iterbon(allread, words, i, window,canvas,question_txt,timeint):
        ...
        if not ansalrgiven:
            ...
            i += 1
            qctr = window.after(int(timeint.get()), lambda: iterbon(allread,words, i,window,canvas,question_txt,timeint))
    root.mainloop()
